...
const CheckoutForm = () => {
  ...
  const handleSubmit = async (ev) => {
    ...
    if (payload.error) {
      ...
      const data = { cart, shipping_fee, total_amount, userId : myUser.id }
      try{
        await axios.post("http://localhost:5000/order/add",data)
        ...
        setError(`An Error has Occured while processing your order}`);
        setProcessing(false);
      }
    }
  };
